...
struct tilemap *cps1_bg_tilemap[3];
...
static void cps1_build_palette(const UINT16* const palette_base);
...
WRITE16_HANDLER( cps1_cps_a_w )
{
    ...
    */
	if (offset == CPS1_PALETTE_BASE)
		cps1_build_palette(cps1_base(CPS1_PALETTE_BASE,cps1_palette_align));
	...
	if (offset > CPS1_VIDEOCONTROL)
		usrintf_showmessage("write to CPS-A register %02x contact MAMEDEV", offset*2);
#endif
}
...
READ16_HANDLER( cps1_cps_b_r )
{
	...
	usrintf_showmessage("CPS-B read port %02x contact MAMEDEV", offset*2);
...
WRITE16_HANDLER( cps1_cps_b_w )
{
	...
	if (cps1_game_config->out2_addr != 0 && offset == cps1_game_config->out2_addr / 2)
	{
		if (ACCESSING_LSB)
		{
...
!cps1_game_config->bootleg_kludge)
		usrintf_showmessage("CPS-B write %04x to port %02x contact MAMEDEV", data, offset*2);
}
...
void cps1_get_video_base(void )
{
	...
	UINT8 kludge = cps1_game_config->bootleg_kludge & 15;
	if (kludge == 0x01)
	{
	...
	if (cps1_game_config->bootleg_kludge == 0x88) // 3wondersb
	{
		scroll1xoff = 0x4;
		scroll2xoff = 0x6;
		scroll3xoff = 0xa;
		cps1_cps_b_regs[0x30/2] = 0x3f;
		cps1_cps_a_regs[CPS1_VIDEOCONTROL] = 0x3e;
		cps1_cps_a_regs[CPS1_SCROLL2_BASE] = 0x90c0;
		cps1_cps_a_regs[CPS1_SCROLL3_BASE] = 0x9100;
		cps1_cps_a_regs[CPS1_PALETTE_BASE] = 0x9140;
	}
...
static int gfxrom_bank_mapper(int type, int code)
{
...
#ifdef MAME_DEBUG
//  usrintf_showmessage("tile %02x/%04x out of range", type,code>>shift);
#endif
...
static void get_tile0_info(int tile_index)
{
	...
	if (code == -1)
		tile_info.pen_data = empty_tile8x8;
}
...
static void get_tile2_info(int tile_index)
{
	...
	SET_TILE_INFO(
			...
			code,
			(attr & 0x1f) + 0x60,
			TILE_FLIPYX((attr & 0x60) >> 5) | TILE_SPLIT
	 ((attr & 0x0180) >> 7))
...
machine_init_cps();
	...
	//stars_rom_size = 0x2000;  /* first 0x4000 of gfx ROM are used, but 0x0000-0x1fff is == 0x2000-0x3fff */
	...
	for (page = 0; page < 6; ++page)
	{
		if (BIT(ctrl,page))
		{
			for (offset = 0; offset < 0x200; ++offset)
			{
				...
				palette_set_color (0x200*page + offset, r, g, b);
			}
		}
		else
		{
...
static void cps1_render_sprites( struct mame_bitmap *bitmap, const struct rectangle *cliprect)
{
...
{																	\
	...
	if (BIT(cps1_game_config->bootleg_kludge, 6))
	{
...
static void cps2_render_sprites(struct mame_bitmap *bitmap,const struct rectangle *cliprect,int *primasks)
{
#define DRAWSPRITE(CODE,COLOR,FLIPX,FLIPY,SX,SY)									\
{																					\
	if (flip_screen)																\
...
static void cps1_render_stars( struct mame_bitmap *bitmap,const struct rectangle *cliprect)
{
	int offs;
...
static void cps1_render_layer(struct mame_bitmap *bitmap,const struct rectangle *cliprect,int layer,int primask)
{
	switch (layer)
	{
		case 0:
...
static void cps1_render_high_layer(struct mame_bitmap *bitmap, const struct rectangle *cliprect, int layer)
{
	switch (layer)
	{
		case 0:
...
VIDEO_UPDATE( cps1 )
{
	...
	if (BIT(cps1_game_config->bootleg_kludge, 7))
		cps1_build_palette(cps1_base(CPS1_PALETTE_BASE, cps1_palette_align));
